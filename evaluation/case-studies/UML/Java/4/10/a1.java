...
{ 
  ...
  { CaseStatement cs = new CaseStatement(); 
    ...
    for (int i = 0; i < ss.size(); i++) 
    { Maplet mm = (Maplet) ss.get(i); 
      Statement stat = ((Statement) mm.dest).removeSlicedParameters(bf,fpars); 
      Maplet nn = new Maplet(mm.source,stat); 
      cs.addCase(nn); 
    } 
    return cs; 
  ...
  { CaseStatement cs = new CaseStatement(); 
    ...
    { Maplet mm = (Maplet) ss.get(i); 
      ...
      Statement stat = cse.addContainerReference(ref,var,excl); 
      Maplet nn = new Maplet(mm.source,stat); 
      cs.addCase(nn); 
    } 
    return cs; 
  ...
  { int n = cases.elements.size();
    for (int i = 0; i < n; i++)
    { Maplet mm = (Maplet) cases.elements.elementAt(i);
      System.out.println("  if (M" + s + "." + s + " == " + ((Named) mm.source).label + ")");
       System.out.print("    { ");
  ...
  { int n = cases.elements.size();
    for (int i = 0; i < n; i++)
    { Maplet mm = (Maplet) cases.elements.elementAt(i);
      out.println("  if (M" + s + "." + s + " == " + ((Named) mm.source).label + ")");
      out.print("    { ");
  ...
  { int n = cases.elements.size();
    ...
    for (int i = 0; i < n; i++)
    { Maplet mm = (Maplet) cases.elements.elementAt(i);
      res = res + "  if (M" + s + "." + s + " == " +  ((Named) mm.source).label + ")";
      res = res + "    {\n";
  ...
  { return; } 
  public void findClones(java.util.Map clones, java.util.Map cdefs, String op, String rule)
  { return; } 
}
