...
def CheckModifier(modifier):
   ...
   return symbol
# ------------------------------------------------------------------------------
def GenerateDotFileForClassesAndInterfaces(Path, JsonFile):
         ...
         if Variables:
            ...
            for variable in Variables:
               ...
               if "Visibility" not in variable or not variable.get("Visibility"):
                  vis1 = "" # "public" I remove it recently 14-6-2025
               else:
               ...
               for Para in Parameters:
                  if FullParameters in ['1','2']:
                     if FirstPar:
                        ...
                        Pa+=','
                  if FullParameters=='1':
                     Pa=Pa+Para["name"]+':'+Para["typeprint"] # Para["type"]
                  if FullParameters=='2':
                     Pa=Pa+Para["typeprint"] # Para["type"]
               Temp=Temp+Pa+') '
...
def GenerateDotFileForRelationships(Path, JsonFile):
   ...
   for item in RelationData:
      Temp=""
      if item["Relationship"] == "Realization": #and not item["Deleted"]:
      ...
      if item["Relationship"] == "Inheritance": #and not item["Deleted"]:
      ...
      if item["Relationship"] == "Composition": #and not item["Deleted"]:
      ...
      if item["Relationship"] == "Aggregation": #and not item["Deleted"]:
      ...
      if item["Relationship"] == "Association": #and not item["Deleted"]:
         if item["Role1"] and item["Role2"] :#item["Flag"]:
            # Bi-association 
      ...
      if item["Relationship"] == "Dependency": #and not item["Deleted"]:
...
if Choice in ['1','2']:
   ...
   if os.path.exists(Path):
      #File = input("Enter the name of file without extension: ")
      #UMLFile=File+".UML"
      #RELFile=File+".REL"
      ...
      if os.path.isfile(FullPath1) and os.path.isfile(FullPath2):
         ...
         GenerateDotFileForClassesAndInterfaces(Path, UMLFile)
      else:
...
if DrawDiagram:
   ...
   Command = 'dot -Tpng "{}{}.dot" -o "{}{}.png"'.format(Path, FileName, Path2, FileName)
   subprocess.run(Command, shell=True)
   print(Path + FileName + '.png')
   Image(filename=Path2 + FileName + '.png')
else:
