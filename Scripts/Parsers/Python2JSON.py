...
def GetArgumentTypes(args):
   ...
   for arg in args:
       ...
       if isinstance(value, (types.FunctionType, types.BuiltinFunctionType)):
          ...
          TypesList.append('OclAny') #'NoneType'
       else:    
   ...
   if ClassN in ClassAttributesInfo:
      ...
      if AttributeIndex != 999:
         ...
         if expected_name in class_info:
            rename_key_preserve_order(class_info, expected_name, Nname)
            return class_info[Nname]
...
def FindReturnStatements(node, FunctionName, localvariables):
   ...
   if isinstance(node, ast.If):
      for child in node.orelse:
         FindReturnStatements(child, FunctionName, localvariables)
# ------------------------------------------------------------------------------
def is_abstract_method(method_node):
   return any(
            ...
            if variables:   
               for variable in variables:
                  ...
                  if IsClassOrInterface(x, Data)=="Class":
                     ClassData = {"Relationship": "Composition", "Source": "", "Target": "", "Multiplicity1": "", "Role1":"", "Multiplicity2": "", "Role2":"", "Deleted": False,"Flag": False }
                     ClassData["Source"] = x
...
# ------------------------------------------------------------------------------
   if item["Relationship"] == 'Composition':
      Data[:] = [
      {**x, 'Deleted': True} if (((x['Source'] == ClassInterface1 and x['Target'] == ClassInterface2) or (x['Source'] == ClassInterface2 and x['Target'] == ClassInterface1)) and (x["Relationship"] != 'Composition')) else x
      for x in Data
      ]
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
def CheckingAggregationAssociation(AggregationInfo, variables, Data, OuterMethodCalls):
   RELResult=[]
   if AggregationInfo!=[]: 
...
def GetVariableType(node):
   if isinstance(node, ast.Str):
      ...
      return "Map"  
   elif isinstance(node, ast.Tuple): # new part
      return "Tuple"       
   else:
      return "unknown"
# ------------------------------------------------------------------------------
               ...
               for statement in BodyNode.body:                 
                  ...
                  for sub_node in node.body:
                     if isinstance(sub_node, ast.ClassDef):  
                        ...
                        break
                  if (
                      not isinstance(statement, (ast.Pass, ast.Return, ast.Expr, ast.Import, ast.If, ast.For, ast.While, ast.AsyncFor, ast.AugAssign, ast.Global))
                      and isinstance(statement.targets[0], ast.Attribute) 
                      and BodyNode.name != "__init__"
                      and isinstance(statement.targets[0].value, ast.Name) 
                      and statement.targets[0].value.id == "self"
                      and isinstance(statement.targets[0].attr, str)
                     ):     
                     MethodData["selfAttributes"].append(statement.targets[0].attr)
...
def ReadPythonFile(Path, File):
   if not os.path.exists(Path):
      ...
      if os.path.exists(FilePath):
         ...
         with open(Program, 'r') as file:
            PythonCode = file.read()
         UMLInfo, variables, AggregationInfo, OuterMethodCalls = ParsePythonCode(CleanPythonCode(PythonCode, False))
         UMLData, RELData= ConstructRelationships(UMLInfo, variables, AggregationInfo, OuterMethodCalls)
         tree = ast.parse(PythonCode)
         MethodSignatures = CollectMethodSignatures(tree)
         Calls = AnalyzeCalls(tree, MethodSignatures)
