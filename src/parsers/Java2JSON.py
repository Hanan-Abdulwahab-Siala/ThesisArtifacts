...
def GetSimpleTypeName(ttt):
   ...
   return ttt.name  
# ------------------------------------------------------------------------------
def ProcessStatements(statements, variables, ParamNames, Context):
   if not statements:
      return
      ...
      if isinstance(stmt, javalang.tree.StatementExpression):
         ...
         if isinstance(expr, javalang.tree.Assignment):
            ...
            for var in variables:
               ...
               if var["name"] == LeftName:
                  ...
                  elif RightName in ParamNames:
                     var["IsParameter"] = True
                     var["WhereParameter"] = Context
                  if Context == "Constructor":
          ...
          ProcessStatements(stmt.statements, variables, ParamNames, Context)
      elif isinstance(stmt, javalang.tree.IfStatement):
          if hasattr(stmt.then_statement, "statements"):
              ProcessStatements(stmt.then_statement.statements, variables, ParamNames, Context)
          if stmt.else_statement:
              if hasattr(stmt.else_statement, "statements"):
                  ProcessStatements(stmt.else_statement.statements, variables, ParamNames, Context)
              else:
                  ProcessStatements([stmt.else_statement], variables, ParamNames, Context)
         ...
         if hasattr(stmt, "body") and stmt.body:
            if hasattr(stmt.body, "statements"):
               ProcessStatements(stmt.body.statements, variables, ParamNames, Context)
            else:
               ProcessStatements([stmt.body], variables, ParamNames, Context)
# ------------------------------------------------------------------------------
# Parsing Java Files
# ------------------------------------------------------------------------------
def ParseJavaCode(JavaCode):
   ...
   for path, node in tree:
      if isinstance(node, javalang.tree.ClassDeclaration):
         ...
         for member in node.body:
            if isinstance(member, (javalang.tree.ConstructorDeclaration, javalang.tree.MethodDeclaration)):
               Context = "Constructor" if isinstance(member, javalang.tree.ConstructorDeclaration) else member.name 
               ParamNames = [p.name for p in member.parameters]
               ProcessStatements(member.body, ClassData["variables"], ParamNames, Context)
            ...
            if variables:
               for variable in variables:
                  ...
                  if IsClassOrInterface(variable["type"], Data): 
                     ...
                     if variable["IsNew"] and variable["WhereParameter"]=="" and variable["WhereNew"]!="" and variable["IsParameter"]==False and (variable["AssignedInConstructor"] or variable["AssignedInMethod"]): 
                        if not IsRelationshipFound("Composition", ClassData["Source"], ClassData["Target"], ClassInfo):
                           ...
                           ClassInfo.append(ClassData) 
                     elif variable["IsParameter"]==True and variable["WhereParameter"]=="Constructor":                        
